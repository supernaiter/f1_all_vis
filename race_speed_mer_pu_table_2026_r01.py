"""
2026 オーストラリアGP — Mercedes PU使用チーム スピード比較テーブル
OTモード除外・DNF除外
"""
...
# Mercedes PUチームのみ
MER_TEAMS = {'Mercedes', 'McLaren', 'Williams', 'Alpine'}

TEAM_COLORS = {
    'Mercedes': '#27F4D2', 'McLaren': '#FF8700',
    'Williams': '#64C4FF', 'Alpine': '#00A1E8',
}
TEAM_BG_COLORS = {
    'Mercedes': '#D5FAF2', 'McLaren': '#FFF0DD',
    'Williams': '#E0F0FF', 'Alpine': '#DCF0FA',
}
TEAM_SHORT = {
    'Mercedes': 'MER', 'McLaren': 'MCL',
    'Williams': 'WIL', 'Alpine': 'ALP',
}
...
# ドライバー別中央値を算出
# チーム別にグループ化し、同チームは隣接配置（v3チャートと整合）
drv_data = {}  # drv -> (team, meds, n)
for drv in results['Abbreviation'].tolist():
    team = drv_team.get(drv, '')
    if team not in MER_TEAMS or drv not in clean:
        continue
    meds = [clean[drv][scol].dropna().median() for scol in SPEED_COLS]
    n = len(clean[drv])
    drv_data[drv] = (team, meds, n)

# チーム順: Mercedes, McLaren, Alpine, Williams（v3チャートのMercedes PUグループ内順序に合わせる）
TEAM_ORDER = ['Mercedes', 'McLaren', 'Alpine', 'Williams']
table_data = []
for team in TEAM_ORDER:
    ...
    for drv in results['Abbreviation'].tolist():
        if drv in drv_data and drv_data[drv][0] == team:
            ...
            table_data.append((drv, t, meds, n))

print(f'Mercedes PUドライバー: {len(table_data)}名')
for drv, team, meds, n in table_data:
    print(f'  {drv} ({TEAM_SHORT[team]}): ST={meds[0]:.1f} FL={meds[1]:.1f} S1={meds[2]:.1f} S2={meds[3]:.1f} n={n}')

# 各列の最大値・最小値
max_per_col = [max(row[2][ci] for row in table_data) for ci in range(4)]
min_per_col = [min(row[2][ci] for row in table_data) for ci in range(4)]
...
pu_avg_meds = []
for ci in range(4):
    pu_avg_meds.append(np.mean([row[2][ci] for row in table_data]))
pu_avg_n = sum(row[3] for row in table_data)
...
# ヘッダー背景
ax.fill_between([0, 14], header_y - 0.45, header_y + 0.55, color='#00B89F', zorder=1)
...
# データ行
prev_team = None
for ri, (drv, team, meds, n) in enumerate(table_data):
    y = total_rows - ri - 0.2
    ...
    # ドライバー名
    ax.text(col_x[0], y, drv, ha='center', va='center',
            fontsize=13, fontweight='bold', color='#1a1a2e', zorder=2)

    # チーム名（チームカラー）
    ax.text(col_x[1], y, TEAM_SHORT[team], ha='center', va='center',
            fontsize=12, fontweight='bold', color=team_color, zorder=2)
    ...
    ax.text(col_x[6], y, str(n), ha='center', va='center',
            fontsize=11, color='#666666', zorder=2)

# PU平均行（データ行の下）
avg_y = total_rows - n_rows - 0.2
# 太い区切り線
ax.axhline(avg_y + 0.45, color='#00B89F', linewidth=1.8, zorder=2)
# 背景
ax.fill_between([0, 14], avg_y - 0.45, avg_y + 0.45, color='#E8F8F5', alpha=0.9, zorder=0)
ax.axhline(avg_y - 0.45, color='#00B89F', linewidth=1.0, zorder=1)

# PU Avg ラベル
ax.text(col_x[0], avg_y, 'PU Avg', ha='center', va='center',
        fontsize=13, fontweight='bold', color='#00B89F', zorder=2)
ax.text(col_x[1], avg_y, f'n={len(table_data)}', ha='center', va='center',
        fontsize=10, fontweight='bold', color='#00B89F', zorder=2)
...
ax.text(col_x[6], avg_y, str(pu_avg_n), ha='center', va='center',
        fontsize=11, fontweight='bold', color='#00B89F', zorder=2)

# 上端罫線
ax.axhline(header_y - 0.45, color='#00B89F', linewidth=1.5, zorder=1)

# タイトル
ax.text(7.0, total_rows + 1.9,
        'Mercedes PU使用チーム スピード比較 (中央値, km/h) — 2026 オーストラリアGP',
...
ax.text(7.0, avg_y - 0.85,
        ...
        '青字 = 各列最速  |  赤字 = 各列最遅\n'
        'Mercedes PU供給: Mercedes, McLaren, Alpine, Williams  |  PU Avg = 全ドライバー中央値の平均',
        ha='center', va='center', fontsize=8.5, color='#888888', style='italic')

plt.subplots_adjust(left=0.02, right=0.98, top=0.86, bottom=0.08)

out = Path('./data/2026_R01_Australia/article/art_speed_mer_pu_comparison.png')
